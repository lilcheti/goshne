...
send(str(os.environ.get("LOC_NAME")))
...
for ghaza in y["data"]["products"]:
  if ghaza["discountRatio"] >= 40:
    ...
    if str(ghaza["productVariationId"]) not in str(f.read()):
      send("["+ghaza["title"]+"](https://m.snappfood.ir/selectSideDish/"+ghaza["productVariationId"]+") "+str(ghaza["price"]-ghaza["discount"]+ghaza["vendorContainerFee"]+int(ghaza["deliveryFee"])))
